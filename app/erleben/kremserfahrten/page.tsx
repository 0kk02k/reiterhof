          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {routen.map((r, i) => (
              <motion.div key={r.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={vp} transition={{ duration: 0.5, delay: i * 0.1 }} className="bg-paper p-10 rounded-xl border border-sand-200 shadow-sm hover:shadow-rustic transition-all group flex flex-col h-full">
                ...
                <motion.a href="/#kontakt" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="block text-center w-full py-4 bg-[#bf9e32] text-sand-100 font-bold rounded-lg hover:bg-[#aa8c2c] active:bg-[#947a27] transition-colors uppercase tracking-widest text-sm shadow-md">Fahrt buchen</motion.a>
              </motion.div>
            ))}
          </div>
        </div>
